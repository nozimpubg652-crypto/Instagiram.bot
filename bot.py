...
async def is_subscribed(user_id: int) -> bool:
    try:
        ...
        logging.error(f"Kanal tekshirishda xato (Bot kanalga admin bo'lishi kerak): {e}")
        return False
...
async def process_insta_link(message: types.Message, state: FSMContext):
    ...
    try:
        # Videoni bloklanmasligi uchun alohida thread'da yuklaymiz
        ...
        os.remove(video_path) # Videoni serverdan o'chirish
    except Exception as e:
...
async def send_broadcast(message: types.Message, state: FSMContext):
    # Bu joyga bazadagi ID larni aylanib chiqish logikasi qo'shiladi
